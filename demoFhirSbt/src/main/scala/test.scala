...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var totalDf = spark.read.option("multiline", true).json("./data/1.json")

    totalDf = flattenDataframe(totalDf)
    totalDf.show()


    val source = Source.fromFile("./data/primer.json")
    for (line <- source.getLines()) {
      val rddDataTemp = spark.sparkContext.parallelize(Seq(line))
      val incrementDf = spark.read.json(rddDataTemp)
      val flatIncrementDf = flattenDataframe(incrementDf)
      totalDf = totalDf.join(flatIncrementDf, Seq("Id"), "outer")
      totalDf.show()
      //val incrementDf = spark.read.json(line)
      incrementDf.show()
    }
    source.close()
  ...
  def partition(df: DataFrame,numPartitions: Int) = {
    var dff = df.withColumn("part", abs(functions.hash(col("id")) % numPartitions))
